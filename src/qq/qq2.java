...
public class qq2 extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (s != null) {
				String q = "select from qq.Weborder2" 
					// Weborder.class.getName()
					//+ " where customer == 'test' && impex==null  order by date";
					+ " where impex=='q' && customer == '"+s+"'";
				
				// String q = "select from " + Weborder.class.getName() +
				// " order by date";

				List<Weborder2> r = (List<Weborder2>) pm.newQuery(q).execute();
	...
	public void set_impex(User user, String impex) {
		PersistenceManager pm = PMF.get().getPersistenceManager();
		Weborder2 w = pm.getObjectById(Weborder2.class, user.getEmail());
		w.set_impex(impex);
	}
